  ...
  */
  ...
  if(frameArguments.length === 2 && typeof frameArguments[0] === 'string' && typeof frameArguments[1] === 'string')
  {
    ...
    if(from !== -1 && to !== -1)
    {
      ...
      //console.log(from.substring(from.length-(4+digits1), from.length-4))
      var number1 = parseInt(from.substring(from.length-(4+digits1), from.length-4), 10); //the 3 used to be 4
      ...
      else
      {
        ...
        else //case: case img1, img2
        {
          ...
          //console.log("from "+prefix1+" "+number1 +" to "+number2);
          for (i = number1; i <= number2; i++) {
            // Use nf() to number format 'i' into four digits
